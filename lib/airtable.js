export async function getPortfolio() {
  ...
  let structuredData = data.records.map((company) => {
    return {
      ...
      status: company.fields["Status"] ? company.fields["Status"] : null,
      ...
      founderPhotos: company.fields["Founder Photos [for website]"]
        ...
        : null,
      simplifiedFund: company.fields["Simplified Fund (website)"]
        ? company.fields["Simplified Fund (website)"]
        : null,
      simplifiedStatus: company.fields["Simplified Status"]
        ? company.fields["Simplified Status"]
        : null,
    };
  });
...
export async function getLFTeam() {
  let myHeaders = new Headers();
  myHeaders.append(
    "Authorization",
    `Bearer ${process.env.NEXT_PUBLIC_AIRTABLE_API_KEY}`
  ...
  const AIRTABLE_URL =
    "https://api.airtable.com/v0/appm10v13QJCjL2PL/Ascension%20Active%20Team?maxRecords=50&view=LifeFundTeamAPI";
  const res = await fetch(AIRTABLE_URL, requestOptions);
  const data = await res.json();
...
export async function getFunds() {
  ...
  let structuredData = data.records.map((fund) => {
    return {
      ...
      headerText: fund.fields["Header Text"]
        ? fund.fields["Header Text"]
        : null,
      headerTextColor: fund.fields["Header Text Color"]
        ? fund.fields["Header Text Color"]
        : null,
      headerBGColor: fund.fields["Header BG Color"]
        ? fund.fields["Header BG Color"]
        : null,
      // maybe add team here as well / focus
    };
  });
...
export async function getPortfolioNewsCompact() {
  ...
  const AIRTABLE_URL =
    "https://api.airtable.com/v0/appm10v13QJCjL2PL/Portfolio%20News?maxRecords=5&view=Approved%20Articles";
  const res = await fetch(AIRTABLE_URL, requestOptions);
  const data = await res.json();
...
export async function getBlogPosts(author) {
  ...
  if (author) {
    ...
    AIRTABLE_URL =
      "https://api.airtable.com/v0/appm10v13QJCjL2PL/Blog?maxRecords=100&view=BlogAPI";
  }
  const res = await fetch(AIRTABLE_URL, requestOptions);
  const data = await res.json();
  ...
  let structuredData = data.records.map((article) => {
    return {
      ...
      snippet: article.fields["Post Snippet"]
        ...
        : null,
      mediumLink: article.fields["Link"] ? article.fields["Link"] : null,
...
export async function getRecentBlogPosts(author) {
  let myHeaders = new Headers();
  myHeaders.append(
    "Authorization",
    `Bearer ${process.env.NEXT_PUBLIC_AIRTABLE_API_KEY}`
  ...
  if (author) {
    ...
    AIRTABLE_URL =
      "https://api.airtable.com/v0/appm10v13QJCjL2PL/Blog?maxRecords=5&view=BlogAPI";
  }
  const res = await fetch(AIRTABLE_URL, requestOptions);
  const data = await res.json();
  ...
  let structuredData = data.records.map((article) => {
    return {
      ...
      author: {
        name: article.fields["Name (from Author)"]
          ? article.fields["Name (from Author)"]
          : null,
        position: article.fields["Position (from Author)"]
          ? article.fields["Position (from Author)"]
          : null,
        twitter: article.fields["Twitter (from Author)"]
          ? article.fields["Twitter (from Author)"]
          : null,
        linkedin: article.fields["LinkedIn (from Author)"]
          ? article.fields["LinkedIn (from Author)"]
          : null,
        photo: article.fields["Picture (from Author)"]
          ? article.fields["Picture (from Author)"][0].url
          : null,
        bio: article.fields["Bio (from Author)"]
          ? article.fields["Bio (from Author)"]
          : "",
        slug: article.fields["Slug (from Author)"]
          ? article.fields["Slug (from Author)"]
          : null,
      },
      date: article.fields["Publish Date"],
      coverImage: article.fields["Cover Image"]
...
export async function getPortfolioJobs() {
  ...
  let AIRTABLE_URL =
    "https://api.airtable.com/v0/appOF73sHhZf7NYZb/Jobs?view=Active%20Jobs";
  const res = await fetch(AIRTABLE_URL, requestOptions);
  const data = await res.json();
...
export async function getLFPortfolio() {
  let myHeaders = new Headers();
  myHeaders.append(
    "Authorization",
    `Bearer ${process.env.NEXT_PUBLIC_AIRTABLE_API_KEY}`
  ...
  const AIRTABLE_URL =
    "https://api.airtable.com/v0/appm10v13QJCjL2PL/Website%20Portfolio?view=LifeFundAPI";
  const res = await fetch(AIRTABLE_URL, requestOptions);
  const data = await res.json();
...
export async function getTestimonials() {
  ...
  const AIRTABLE_URL =
    "https://api.airtable.com/v0/appm10v13QJCjL2PL/Testimonials?view=testimonialAPI";
  const res = await fetch(AIRTABLE_URL, requestOptions);
  const data = await res.json();

  // problem with the founder photo

  let structuredData = data.records.map((testimonial) => {
    return {
      company: testimonial.fields["Company Name"][0],
      slug: testimonial.fields["slug"] ? testimonial.fields["slug"][0] : "/",
      quote: testimonial.fields["Quote"] ? testimonial.fields["Quote"] : null,
      founderPhoto: testimonial.fields["Profile Picture URL (from Founder)"][0],
      companyLogo: testimonial.fields["Company Logo"][0].url,
      founderName: testimonial.fields["Founder Name"][0],
      title: testimonial.fields["Founder Title"][0],
    };
  });
