export async function getPortfolio() {
  ...
  let structuredData = data.records.map((company) => {
    // const item = await getImageLowRes(company.fields.Logo[0].url);
    return {
...
export async function getBlogPosts(author) {
  ...
  if (author) {
    ...
    AIRTABLE_URL =
      "https://api.airtable.com/v0/appm10v13QJCjL2PL/Blog?maxRecords=3&view=Post%20Management";
  }
  const res = await fetch(AIRTABLE_URL, requestOptions);
  const data = await res.json();
